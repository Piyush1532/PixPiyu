...
<div className='
max-w-lg h-74 pb-16 sm:pb-10 pt-2 relative'>
  <div ref={editorRef}></div>
<button type='button' onClick={generateContent} className='absolute rounded-1 right-2 ml-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded hover:underline cursor-pointer'>Generate with AI</button>
</div>

<p className='mt-4'>Blog Category </p>
<select name="category"className='mt-2 px-3 py-2 border text-gray-500 border-gray-300 outline-none rounded' onChange={(e)=>setCategory(e.target.value)} >
...
<button type='submit' className='mt-8 w-40 h-10 bg-[#3EA6A9] text-white rounded cursor-pointer text-sm' disabled={isAdding}>{isAdding ? "Adding...":"Add Blog"}</button>
